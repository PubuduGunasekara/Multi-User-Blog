...
import { createMCategory, mCategoryList, removeMCategory } from '../../actions/mobileCategory.action';
...
const CreateMcategoryComponent = ({ router }) => {
    ...
    const publihMCategory = (e) => {
        e.preventDefault();
        createMCategory(formData, token)
            .then(data => {
                if (data.error) {
                    setValues({ ...values, error: data.error })
                } else {
                    setValues({ ...values, name: '', body: '', error: '', success: `A new Mobile Category "${data.name}" is created` });
                    setPhoto('');
                    setImgBtn(false);
                    loadMCategories();
                }
    ...
    const showError = () => (
        <div className="alert alert-danger container-fluid" style={{ display: error ? '' : 'none' }}>
            {error}
        </div>
    );

    const showSuccess = () => (
        <div className="alert alert-success container-fluid" style={{ display: success ? '' : 'none' }}>
            {success}
        </div>
    );
    ...
    const createMCategoryForm = () => {
        return (
            <form className="container-fluid" onSubmit={publihMCategory}>
                ...
                <div>
                    <Button className="submit" className="btn btn-block btn-lg" >Add</Button>
                </div>
        ...
        return mCategories.map((t, i) => {
            return (
                <div key={i} style={{ border: 'red' }}>
                    <a
                        onClick={() => { Router.replace(`/admin-panel/root-admin/crud/mobile-category/update/${t.slug}`); }}
                        title="click to update"
                        className="btn btn-outline-primary mr-1 ml-1 mt-1"
                    >{t.name}</a><a onClick={() => deleteConfirm(t.slug)} title="click to Delete" className="btn btn-danger">X</a>
                </div>
            )
    ...
    return (
        <div className="container-fluid">
            <div className="row" style={{ width: '100%', paddingLeft: 0, paddingRight: 0, marginLeft: 0, marginRight: 0 }}>
                <div className="col-md-8" style={{ width: '100%', paddingLeft: 0, paddingRight: 0, marginLeft: 0, marginRight: 0 }}>
                    {showError()}
                    {showSuccess()}
                    {createMCategoryForm()}

                </div>
                <div className="col-md-4" style={{ width: '100%', paddingLeft: 0, paddingRight: 0, marginLeft: 0, marginRight: 0 }}>
