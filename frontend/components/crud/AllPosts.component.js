...
import { reviewListPrivate, reviewListPublic } from '../../actions/review.action';
import { newsListPrivate, newsListPublic } from '../../actions/news.action';
import { mobileListPrivate, mobileListPublic } from '../../actions/mobile.action';
...
const AllPost = () => {
    ...
    //pagination
    const indexofLastPostMobilePrivate = currentPageMobilePrivate * postPerPageMobilePrivate;
    const indexofFirstPostMobilePrivate = indexofLastPostMobilePrivate - postPerPageMobilePrivate;
    const currentPostMobilePrivate = mobilePrivate.slice(indexofFirstPostMobilePrivate, indexofLastPostMobilePrivate);
    ...
    //pagination
    const indexofLastPostMobilePublic = currentPageMobilePublic * postPerPageMobilePublic;
    const indexofFirstPostMobilePublic = indexofLastPostMobilePublic - postPerPageMobilePublic;
    const currentPostMobilePublic = mobilePublic.slice(indexofFirstPostMobilePublic, indexofLastPostMobilePublic);
                        ...
                        <Row className="mr-0 ml-0">
                            <Col md="12">
                                ...
                                <div className="row mr-0 ml-0">
                                    ...
                                    <div className="col-md-6">
                                        <h5 className="text-center mt-2 pt-4 pb-4">private mobiles - ({mobilePrivate.length})</h5>
                                        {showPrivateMobiles()}
                                        {Pagination(postPerPageMobilePrivate, mobilePrivate.length, paginateMobilePrivate, nextPageMobilePrivate, previousPageMobilePrivate, currentPageMobilePrivate, lastPageMobilePrivate)}
                                    </div>
                                    <div className="col-md-6">
                                        <h5 className="text-center mt-2 pt-4 pb-4">public mobiles - ({mobilePublic.length})</h5>
                                        {showPublicMobiles()}
                                        {Pagination(postPerPageMobilePublic, mobilePublic.length, paginateMobilePublic, nextPageMobilePublic, previousPageMobilePublic, currentPageMobilePublic, lastPageMobilePublic)}
                                    </div>
                                </div>
                            </Col>
                        </Row>
    ...
    const showPublicMobiles = () => {
        return currentPostMobilePublic.map((m, i) => {
            if (m.flag === 1) {
                            ...
                            <div className="card">
                                <div className="row card-body">
                                    <div className="col-md-8">
                                        ...
                                        <p className="card-text">
                                            Written by {m.postedBy.name} | Published on {moment(m.updatedAt).fromNow()}
                                        </p>
                                        {showViewButtonMobile(m)}
                                    </div>
                                    < img src={`${API}/mobile/photo/${m.slug}`}
                                        className="img img-fluid pt-3 col-md-4"
                                        alt="news image cap"
                                    />
                                </div>
                ...
                return <div key={i} className="alert alert-info"><p>No <kbd>public</kbd> mobiles at the moment</p></div>
    ...
    const showPrivateMobiles = () => {
        return currentPostMobilePrivate.map((m, i) => {
            if (m.flag === 0) {
                ...
                return <div key={i} className="alert alert-info"><p>No <code>private</code> mobiles at the moment</p></div>
