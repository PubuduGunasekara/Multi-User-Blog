 ...
 * completed!
 */
const CreateReviewComponent = ({ router }) => {

     /**
     * reviews restored from local storage
     */
    const reviewFromLS = () => {
        ...
        if (localStorage.getItem(`review-${isAuth()._id}`)) {
            return JSON.parse(localStorage.getItem(`review-${isAuth()._id}`))
        } else {
            return ''
        }
    };
    ...
    const [body, setBody] = useState(reviewFromLS());
        ...
        createReview(formData, token)
            .then(data => {
                if (data.error) {
                    ...
                    setValues({ ...values, title: '', error: '', success: `A new Review Titled "${data.title}" is created` });
                    setBody('');
                    setTags([]);
                    setPhoto('');
                    setImgBtn(false);
                    if (process.browser) {
                        localStorage.removeItem(`review-${isAuth()._id}`);
                    }
                    initTags();
                }
            });
    ...
    const handleBody = (e) => {
        ...
        formData.set('body', e)
        if (typeof window !== 'undefined') {
            localStorage.setItem(`review-${isAuth()._id}`, JSON.stringify(e))
        }
    };
    ...
    const createReviewForm = () => {
        return (
            <form onSubmit={publihReview}>
                ...
                <div>
                    <Button className="submit" className="btn btn-block btn-lg" >Publish</Button>
                </div>
...
export default withRouter(CreateReviewComponent);