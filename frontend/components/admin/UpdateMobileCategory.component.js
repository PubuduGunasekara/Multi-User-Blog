...
import { singleMCategory, updateMCategory } from '../../actions/mobileCategory.action';
...
const MCatgoryUpdate = ({ router }) => {
    ...
    const editMCategory = (e) => {
        ...
        updateMCategory(formData, token, router.query.slug).then(data => {
            if (data.error) {
                ...
                setValues({ ...values, name: '', body: '', success: `A Categorry named "${data.name}" is successfully updated` });
    ...
    const showSuccess = () => (
        <div className="alert alert-success" style={{ display: success ? '' : 'none' }}>
            {success}
            <a href={`${DOMAIN}/admin-panel/root-admin/crud/mobile-category`}>. click here</a> to go back
        </div>
    );
                ...
                <div className="col-md-4 mb-5">
                    <div>
                        <div className="form-group pb-2">
                            ...
                            {body && <img className="img img-fluid" src={`${API}/mobile-category/photo/${router.query.slug}`} alt={name} style={{ width: '100%' }}></img>}
                            <hr />
