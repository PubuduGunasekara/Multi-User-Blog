...
const NewsUpdate = ({ router }) => {
        ...
        return (
            tags && tags.map((t, i) => (
                <li key={i} className="list-unstyled">
                    <input onChange={handleTagsToggle(t._id)} checked={findOutTags(t._id)} type="checkbox" className="mr-2" />
    ...
    const editNews = (e) => {
        e.preventDefault();
        updateNews(formData, token, router.query.slug).then(data => {
            if (data.error) {
                setValues({ ...values, error: data.error })
            } else {
                setValues({ ...values, title: '', success: `A News Titled "${data.title}" is successfully updated` });
            }
        })
    };
    ...
    const showSuccess = () => (
        <div className="alert alert-success" style={{ display: success ? '' : 'none' }}>
            {success}
            <a href={`${DOMAIN}/admin-panel/root-admin/list-all-post`}>. click here</a> to go back
        </div>
    );
    ...
    const updateNewsForm = () => {
        return (
            <form onSubmit={editNews}>
                ...
                <div>
                    <Button className="submit" className="btn btn-block btn-lg" >Update</Button>
                </div>
                ...
                <div className="col-md-4 mb-5">
                    <div>
                        <div className="form-group pb-2">
                            ...
                            {body && <img className="img img-fluid" src={`${API}/news/photo/${router.query.slug}`} alt={title} style={{ width: '100%' }}></img>}
                            <hr />
