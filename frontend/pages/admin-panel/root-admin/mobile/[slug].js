...
import { singleMobileCreators } from "../../../../actions/mobile.action";
...
const SingleBlog = ({ mobile, query }) => {
  ...
  const approveButton = async () => {
    ...
    if (ask) {
      approveMobile(mobile.slug, token).then((data) => {
        if (data.error) {
          setMobilePublicValues({
            ...mobilePublicValues,
  ...
  const unPublishButton = async () => {
    ...
    if (ask) {
      unPublishMobile(mobile.slug, token).then((data) => {
        if (data.error) {
          setMobilePublicValues({
            ...mobilePublicValues,
            postError: data.error,
  ...
  const showUpdatedButton = (slug) => {
    return (
      <a
        href={`${DOMAIN}/admin-panel/root-admin/mobile/update/${slug}`}
        ...
        <div className="row ml-0 mr-0">
          <div className="col-md-12">
            ...
            <button
              ...
              onClick={() => deleteConfirm(mobile.slug)}
            >
              Delete
            </button>

            {showUpdatedButton(mobile.slug)}
                ...
                <div className="mt-5 mb-5">
                  <section>
                    <div className="row" style={{ marginTop: "-30px" }}>
                      <img
                        src={`${API}/mobile/photo/${mobile.slug}`}
                        alt={mobile.title}
                  ...
                  <section>
                    <div>
                      ...
                      <p className="lead mt-3 mark">
                        Written by {mobile.postedBy.username} | published{" "}
                        {moment(mobile.updatedAt).fromNow()}
                      </p>
                    </div>
                  </section>
                </div>
                      ...
                      <table className="table table-striped table-hover">
                        <tbody>
                          ...
                          <tr>
                            ...
                            >
                              <div>
                                <h4 className="mt-3 mb-3 font-weight-bolder">
                                  Display
                                </h4>
                              </div>
                            </td>
...
SingleBlog.getInitialProps = ({ query }) => {
  return singleMobileCreators(query.slug).then((data) => {
    if (data.error) {
      console.log(data.error);
    } else {
